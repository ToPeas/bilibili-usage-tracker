...
chrome.alarms.onAlarm.addListener(async alarm => {
  if (alarm.name === "daily-upload") {
    await flushActiveSegment(Date.now());
    await uploadPendingDays();
    await scheduleDailyUpload();
  }
});
...
chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
  (async () => {
    ...
    if (message?.type === "bili-usage-delta") {
      const result = await handleUsageDelta(message, _sender);
      sendResponse(result);
      return;
    }
    if (message?.type === "bili-heartbeat") {
      const result = await handleLegacyHeartbeat(message, _sender);
      sendResponse(result);
      return;
    }
    if (message?.type === "upload-now") {
      await flushActiveSegment(Date.now(), { keepRunning: true });
      const result = await uploadPendingDays({ includeToday: true });
      sendResponse({ ok: true, result });
      return;
    }
    if (message?.type === "get-recent-usage") {
      sendResponse(await getRecentUsage());
      return;
    }
...
async function reconcile() {
  ...
  const shouldTrack = TARGET_HOSTS.has(context.host);

  state.active = shouldTrack;
  state.host = shouldTrack ? context.host : "";
  state.tabId = shouldTrack ? context.tabId : 0;
  state.lastDeltaAt = shouldTrack ? state.lastDeltaAt : 0;
}

async function flushActiveSegment(now, options = {}) {
...
async function handleLegacyHeartbeat(_message, _sender) {
...
async function addDuration(host, startMs, endMs) {
  ...
  let cursor = startMs;
  while (cursor < endMs) {
    const next = Math.min(endMs, nextLocalMidnight(cursor));
    const date = formatLocalDate(new Date(cursor));
    usage[date] = usage[date] || {};
    usage[date][host] = Math.max(0, Math.round((usage[date][host] || 0) + (next - cursor)));
    cursor = next;
  }
...
async function uploadPendingDays(options = {}) {
  ...
  if (!schema.ok) {
    uploadLog.unshift({ date: "schema", ok: false, status: 0, message: schema.error.slice(0, 200), time: Date.now() });
  ...
  const dates = Object.keys(usage)
    .filter(date => (options.includeToday || date !== today) && (options.includeToday && date === today ? true : !uploaded[date]))
    .sort()
    .slice(-30);

  let uploadedCount = 0;
  for (const date of dates) {
    const items = Object.entries(usage[date] || {})
    ...
    const payload = {
      ...
      items,
      totalMs: items.reduce((sum, item) => sum + item.durationMs, 0),
    ...
    if (!cloudflareOk) {
      const message = result?.errors?.map(error => error.message).join("; ") || response.statusText || "D1 query failed";
      uploadLog.unshift({ date, ok: false, status: response.status, message: message.slice(0, 200), time: Date.now() });
      break;
    }

    if (date !== today || options.includeToday) {
      uploaded[date] = true;
    }
    uploadedCount += 1;
  ...
  await chrome.storage.local.set({ uploaded, uploadLog: uploadLog.slice(0, 50) });
...
async function getRecentUsage() {
  ...
  if (!settings.accountId || !settings.databaseId || !settings.cloudflareApiToken) {
    return {
      ...
      database: getDatabaseLabel(settings),
      days: []
    };
  }
...
async function testD1Connection() {
  ...
  if (!settings.accountId || !settings.databaseId || !settings.cloudflareApiToken) {
    return {
      ok: false,
      database: getDatabaseLabel(settings),
      error: "请先填写 Account ID、Database ID 和 API Token"
    };
  }

  const readTest = await queryD1(settings, {
    sql: "SELECT 1 AS ok",
    params: []
  });
  if (!readTest.ok) {
...
async function ensureD1Schema(settings) {
  const createResult = await queryD1(settings, {
    batch: buildD1SchemaBatch()
  });
  if (!createResult.ok) return createResult;

  const columns = await queryD1(settings, {
    sql: "PRAGMA table_info(usage_daily)",
    params: []
  });
  if (!columns.ok) return columns;

  const hasDeviceAlias = columns.rows.some(row => row.name === "device_alias");
  if (hasDeviceAlias) return { ok: true };

  return queryD1(settings, {
...
function buildD1SchemaBatch() {
  return [
    ...
    {
      sql: "CREATE INDEX IF NOT EXISTS idx_usage_daily_date ON usage_daily(date)",
      params: []
    },
    {
      sql: "CREATE INDEX IF NOT EXISTS idx_usage_items_bundle ON usage_items(bundle)",
      params: []
    }
  ];
}
...
async function getStatus() {
  ...
  return {
    ...
    host: state.host,
    today,
    todayTotalMs: Object.values(todayItems).reduce((sum, value) => sum + value, 0),
    todayItems,
    uploaded,
